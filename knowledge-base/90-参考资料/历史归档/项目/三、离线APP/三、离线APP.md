#### 一、什么是离线APP?
离线APP是支持即使在无网络的场景下，也能够正常使用的APP。



#### 二、如何实现离线场景下，也能正常运行程序
+ 代码的离线和更新
+ 数据的下载、上传、更新
...
#### 三、代码的离线和更新
##### 1.离线
web 应用不管是网页还是 H5，通常都是在线服务，代码都需要部署到线上服务器，但是离线应用就不能只依赖于网络，在没有网络的场景下，也需要想办法让用户的客户端可以获取到程序代码，这就需要依赖原生 APP 开发的能力了，采用是web + Cordova 的跨平台APP方案。
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698310609140-51f546b8-1062-4ad1-b7af-86f461dac443.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698376456154-9d6646f7-f50a-4036-9279-078d3050da3a.png)
...
可以简单称呼两种方案为：**大版本更新**和**热更新**



其实，更准确来讲，前端要自行实现的热更新机制包括两个方面：**web 热更新**和 **db 热更新**，前者是更新 web 代码文件，后者是更新 db 数据库表
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698377103897-d98fd2e5-4ab5-40de-8257-07939d058994.png)
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698389378767-457021dd-cae1-45f8-9871-6a5b4964c8a1.png)

这个插件会调用原生的下载能力去下载 web 代码文件，下载完成后，再解压到 APP 应用的存放 web 代码文件的目录里

需要注意的是，这个插件只进行解压操作，意思也就是，不会删除原有的 web 代码文件，除非同目录下同名文件，才会进行覆盖操作，这也是为什么超级 APP 可以实现各个子应用间独立热更新的原因，因为只要隔离好各个子应用的 web 代码文件，那各个子应用的热更新文件就不会有冲突，也就不会相互影响

这就是 web 热更新的大体思路

![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698389878083-e24e5ba5-cb3d-428c-ac7b-c5a6138ea5e7.png)
...
存自然就是要持久化存储，但对于前端来说，在持久化存储方面的能力并不是很强，localstorage 有大小限制；indexedDB 是非关系型数据库，且存在兼容性问题，所以离线APP方案在存储方面采用的是通过 Cordova 插件调用原生能力来存储在 sqlite 的 db 文件中

sqlite 是原生 APP 支持的数据库存储方案，语法方面跟 mysql 基本一样，存储后就是一份 db 文件，调试时可直接通过 Cordova 插件执行 sql 查询数据。
...
<font style="color:#DF2A3F;">上传的时机通常有自动上传和手动上传两种，自动上传需要考虑的场景比较多，比如有网无网、数据有效无效等等，所以实现上更多的还是倾向于交给用户手动上传。</font>
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698820021771-190f80ef-674e-49a1-a4be-d051af07bad5.png)
...
APP 是根据用户来建数据库的，不同用户有不同的数据库，这样就能确保在 APP 内切换用户使用时，相互间互不影响
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698820375680-8b0881e3-e42f-4f72-aff7-6f6d4380999f.png)
...
从业务来看的话，表数据大概可以分为两类：基础数据和业务数据，基础数据又可以分为系统级基础数据、租户级基础数据、项目级基础数据
...
![](https://cdn.nlark.com/yuque/0/2023/png/25743026/1698820875390-f7d2d135-5aae-4e48-b1ba-ed7612f99a9a.png)  
